...
func Test_spatial_grid_Insert(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			params: []params{
				{item: 1, bounds: data.NewRectangle(data.Vector{X: 4, Y: 4}, 2, 2)},
				{item: 2, bounds: data.NewRectangle(data.Vector{X: 12, Y: 12}, 2, 2)},
				{item: 3, bounds: data.NewRectangle(data.Vector{X: 20, Y: 20}, 2, 2)},
				{item: 4, bounds: data.NewRectangle(data.Vector{X: 28, Y: 28}, 2, 2)},
			},
			wants: []wants{
				{x: 0, y: 0, item: 1},
				{x: 1, y: 1, item: 2},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sg := data.NewSpatialGrid[int](tt.fields.x, tt.fields.y, tt.fields.size)

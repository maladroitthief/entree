...
type Level struct {
	...
	player       canvas.Entity
	width        int
	height       int
	size         int
}

func NewLevel(rf RoomFactory, bf BlockFactory, player canvas.Entity) *Level {
	l := &Level{
		roomFactory:  rf,
		blockFactory: bf,
		player:       player,
	...
	for y := 0; y < len(l.Rooms); y++ {
		for x := 0; x < len(l.Rooms[y]); x++ {
			for i, block := range l.Rooms[y][x].layout {
				switch block {
				case Player:
					l.blockFactory.AddPlayer(c, l.player, xPosition(x, i), yPosition(y, i))
				case EmptySpace:
				case Solid:
					l.blockFactory.AddWall(c, xPosition(x, i), yPosition(y, i))
				}
			}
		}
	}
